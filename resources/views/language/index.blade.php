        ...
        <table class="table" id="data_table">
          ...
          @foreach($data as $row)
            <tr>
              ...
              <td><img class="rounded-circle" src="@if($row->image) @if(App\Models\StorageSetting::getStorageSetting('storage') == 'DigitalOcean'){{\Storage::disk('spaces')->url('uploads/'.$row->image)}} @else {{asset('uploads/'.$row->image)}} @endif @else {{asset('assets/images/no-image.png')}} @endif" width="50px" height="50px"><b class="ml-3" style="font-size:16px;">{{$row->title}}</b></td>
              ...
              <td class="align-middle">
                <div class="btn-group">
                  <a href="{{url('admin/language/'.$row->id.'/edit') }}"><button type="button" class="btn btn-success"><span aria-hidden="true" class="fa fa-edit"></span></button></a>
                ...
                {!! Form::open(['url' => 'admin/language/'.$row->id,'method'=>'DELETE','class'=>'form-horizontal','id'=>'form_'.$row->id]) !!}
