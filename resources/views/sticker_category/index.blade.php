  ...
  <div class="col-md-12">
    ...
    <div class="card card-primary">
        <div class="card-header">
            ...
            <a href="{{ route('sticker-category.create')}}" class="btn btn-success float-right">Add New</a>
        </div> 
        ...
        <table class="table" id="data_table">
          ...
          @foreach($data as $row)
            <tr>
              ...
              <td class="align-middle"><b class="ml-3" style="font-size:16px;">{{$row->name}}</b></td>
              ...
              <td class="align-middle">
                <div class="btn-group text-center">
                    <a href="{{url('admin/sticker-category/'.$row->id.'/edit') }}"><button type="button" class="btn btn-success"><span aria-hidden="true" class="fa fa-edit"></span></button></a>
                ...
                {!! Form::open(['url' => 'admin/sticker-category/'.$row->id,'method'=>'DELETE','class'=>'form-horizontal','id'=>'form_'.$row->id]) !!}
...
<script type="text/javascript">
    ...
    $(".status").change(function(){
      ...
      $.ajax({
        type: "POST",
        url: "{{url('admin/sticker-category-status')}}",
        data: { checked : checked , id : id},
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        success: function(data) {
          new PNotify({
            title: 'Success!',
            text: "Sticker Category Status Has Been Changed.",
            type: 'success'
          });
        },
      });
