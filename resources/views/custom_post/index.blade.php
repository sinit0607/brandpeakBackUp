        ...
        <table class="table" id="data_table">
          ...
          @foreach($data as $row)
            <tr>
              ...
              <td class="align-middle">
                <label class="switch mt-3">
                  @php $post = App\Models\FeaturePost::where("custom_id",$row->id)->get(); @endphp
...
<script type="text/javascript">
    ...
    $(".status").change(function(){
      ...
      $.ajax({
        ...
        success: function(data) {
          new PNotify({
            title: 'Success!',
            text: "Custom Post Status Has Been Changed.",
            type: 'success'
          });
        },
      });
    ...
    $(".feature_status").change(function(){
      ...
      $.ajax({
        type: "POST",
        url: "{{url('admin/custom-feature-status')}}",
        data: { checked : checked , id : id},
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        success: function(data) {
          if(data == 1){
            new PNotify({
              title: 'Success!',
              text: "Custom Category Feature Set!.",
              type: 'success'
            });
          }
          else
          {
            new PNotify({
              title: 'Success!',
              text: "Custom Category Feature Unset!.",
              type: 'success'
            });
          }
        },
