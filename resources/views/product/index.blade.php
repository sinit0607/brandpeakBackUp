        ...
        <table class="table" id="data_table">
          ...
          @foreach($data as $row)
            <tr>
              ...
              <td><img class="rounded-circle" src="@if(App\Models\StorageSetting::getStorageSetting('storage') == 'DigitalOcean'){{\Storage::disk('spaces')->url('uploads/'.$row->image)}} @else {{asset('uploads/'.$row->image)}} @endif" width="50px" height="50px"><b class="ml-3" style="font-size:16px;">{{$row->title}}</b></td>
              <td class="align-middle">{{$row->ProductCategory->name}}</td>
              <td class="align-middle">{{$row->price}}</td>
              <td class="align-middle">{{$row->discount_price}}</td>
              ...
              <td class="align-middle">
                <div class="btn-group text-center">
                    <a href="{{url('admin/product/'.$row->id.'/edit') }}"><button type="button" class="btn btn-success"><span aria-hidden="true" class="fa fa-edit"></span></button></a>
